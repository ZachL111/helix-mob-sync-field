...
`helix-mob-sync-field` is centered on mobile workflows work in Go. The technical target is: Create a Go reference implementation for sync workflows, centered on policy evaluation, deny and allow fixtures, and explainable decision traces.
...
The score is deterministic. The current threshold is 172. Risk has a penalty of 5, latency has a penalty of 3, and weight contributes a bonus of 6.
